...
**Anbefaling (kræver en beslutning fra Christian, ikke bygget endnu):**
1. **Tilføj Google Books' Volumes API som ekstra kilde** ved siden af Open Library i bogsøgningen ("+ Tilføj bog"). Den er gratis, kræver ingen nøgle for almindeligt forbrug, og har ofte bedre/nyere dækning for internationale (herunder danske) udgivelser end Open Library, fordi data kommer fra forlag/boghandlere globalt. Lav risiko, lille kodeændring (samme mønster som `book_service.py`'s Open Library-kald i Atlas 1, blot en ekstra fetch og et fallback-forsøg).
2. **Undgå at hotlinke billeder direkte fra saxo.dk.** Det er ikke en API bygget til formålet — billed-URL'er kan ændre sig uden varsel, mange forhandlere blokerer "hotlinking" (billedvisning fra andre sites) via en Referer-header-tjek, og det er en uafklaret gråzone at genbruge en forhandlers produktbilleder på et andet site, selv et privat et. Det er ikke et stabilt fundament at bygge videre på.
3. **Den mest robuste løsning uanset kilde er allerede i appen:** manuel cover-upload findes allerede på "+ Tilføj bog"/redigér-formularen. For de bøger hverken Open Library eller Google Books rammer, er et hurtigt eget foto/skærmbillede uploadet direkte den løsning, der aldrig knækker bagefter (data ligger i egen database, ikke afhængig af en ekstern URLs levetid) — værd at gøre til den anbefalede vej for "besværlige" bøger, fremfor at jagte flere og flere eksterne kilder.

Ikke implementeret endnu, da det er en tilføjelse af en ny ekstern afhængighed (om end nøglefri) og derfor værd at bekræfte med Christian først.
